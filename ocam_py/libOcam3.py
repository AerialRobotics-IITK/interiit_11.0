...
import os
import v4l2py
from v4l2py.raw import v4l2_capability
import v4l2py.raw as v4l2raw
import fcntl
import mmap
import time
...
class v4l2_util:
    def __init__(self, devpath, verbose=False):
        ...
        try:
            self.vdr = open(devpath, "r")
            self.vdw = open(devpath, "w")
        except:
            raise IOError("Device Open Error %s" % devpath)
        if self.verbose:
            print("--Device Open Success!!!")
            print("  + Device:", self.vdr)
    ...
    def Close(self):
        ...
        self.vdw.close()
        if self.verbose:
            print("--Device Closed", self.vdr)

    def Capability(self):
        cp = v4l2_capability()
        xioctl(self.vdr, v4l2raw.VIDIOC_QUERYCAP, cp)
        if self.verbose:
            print("--Capability ")
            ...
            print(
                "  + VIDEO_CAPture:",
                hex(cp.capabilities & v4l2raw.V4L2_CAP_VIDEO_CAPTURE),
            )
            print("  + STREAMING", hex(cp.capabilities & v4l2raw.V4L2_CAP_STREAMING))
        return cp.card
    ...
    def FormatDescript(self):
        ...
        fmt = v4l2raw.v4l2_fmtdesc()
        fmt.type = v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE
        for i in range(20):  # while True:
            try:
                fmt.index = i
                xioctl(self.vdw, v4l2raw.VIDIOC_ENUM_FMT, fmt)
            except:
                break
            if self.verbose:
    ...
    def EnumerateFormats(self):
        ...
        for key in fmtdesc:
            fse = v4l2raw.v4l2_frmsizeenum()
            for i in range(10):
                fse.index = i
                fse.pixel_format = fmtdesc[key]
                try:
                    xioctl(self.vdw, v4l2raw.VIDIOC_ENUM_FRAMESIZES, fse)
                except:
                    break
                fpslist = self.GetFrameRate(
                    fse.pixel_format, fse.discrete.width, fse.discrete.height
                )
                for fps in fpslist:
                    self.listValidFormats.append(
                        (key, fse.discrete.width, fse.discrete.height, fps)
                    )

        return self.listValidFormats

    def GetFrameRate(self, pixelformat, width, height):
        frmival = v4l2raw.v4l2_frmivalenum()
        ...
        for i in range(10):
            frmival.index = i
            try:
                xioctl(self.vdw, v4l2raw.VIDIOC_ENUM_FRAMEINTERVALS, frmival)
            except:
                break
            if frmival.type == v4l2raw.V4L2_FRMIVAL_TYPE_DISCRETE:
                fps = int(
                    1.0
                    / (
                ...
                fps_list.append(fps)
            elif frmival.type == v4l2raw.V4L2_FRMIVAL_TYPE_CONTINUOUS:
                print("todo V4L2_FRMIVAL_TYPE_CONTINUOUS")
            elif frmival.type == v4l2raw.V4L2_FRMIVAL_TYPE_STEPWISE:
    ...
    def EnumerateControls(self):
        ...
        qc = v4l2raw.v4l2_queryctrl()
        qc.id = v4l2raw.V4L2_CTRL_CLASS_USER | v4l2raw.V4L2_CTRL_FLAG_NEXT_CTRL
        if self.verbose:
            print("--EnumerateControls")
        for i in range(20):  # while True:
            try:
                xioctl(self.vdr, v4l2raw.VIDIOC_QUERYCTRL, qc)
            except:
                break
            if self.verbose:
                print("  +", i, ":", qc.id, qc.name, qc.type, qc.flags)

            if qc.flags == v4l2raw.V4L2_CTRL_FLAG_DISABLED:
                continue
            if qc.type == v4l2raw.V4L2_CTRL_TYPE_MENU:
                print("sdkim Todo enumerate_control_menu() ")
            self.dictValidControls[qc.name] = qc.id
            qc.id |= v4l2raw.V4L2_CTRL_FLAG_NEXT_CTRL
        return self.dictValidControls

    def GetControlValue(self, qcid):
        ctrl = v4l2raw.v4l2_control()
        ctrl.id = qcid
        xioctl(self.vdr, v4l2raw.VIDIOC_G_CTRL, ctrl)
        if self.verbose:
            print("--GetControl")
            print(" + ID:", ctrl.id)
    ...
    def SetControlValue(self, qcid, val):
        ...
        ctrl.id = qcid
        ctrl.value = val
        xioctl(self.vdw, v4l2raw.VIDIOC_S_CTRL, ctrl)
        if self.verbose:
    ...
    def SetFormat(self, form, stp):
        xioctl(self.vdw, v4l2raw.VIDIOC_S_FMT, form)
        self.shape = (form.fmt.pix.height, form.fmt.pix.width)
        xioctl(self.vdw, v4l2raw.VIDIOC_S_PARM, stp)
        self.fps = (
    ...
    def GetCurrentFormat(self):
        form = v4l2raw.v4l2_format()
        form.type = v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE
        xioctl(self.vdr, v4l2raw.VIDIOC_G_FMT, form)

        self.shape = (form.fmt.pix.height, form.fmt.pix.width)  ##### self.shape

        stp = v4l2raw.v4l2_streamparm()
        stp.type = v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE
        xioctl(self.vdr, v4l2raw.VIDIOC_G_PARM, stp)
        self.fps = (
            stp.parm.capture.timeperframe.denominator
            / stp.parm.capture.timeperframe.numerator
        )
    ...
    def Start(self, buff_num):
        if self.verbose:
            print("==== START ====")
        req = v4l2raw.v4l2_requestbuffers()
        req.count = buff_num
        req.type = v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE
        req.memory = v4l2raw.V4L2_MEMORY_MMAP
        xioctl(self.vdr, v4l2raw.VIDIOC_REQBUFS, req)
        ...
        for i in range(n_buffers):
            buf = v4l2raw.v4l2_buffer()
            buf.type = v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE
            buf.memory = v4l2raw.V4L2_MEMORY_MMAP
            buf.index = i
            xioctl(self.vdr, v4l2raw.VIDIOC_QUERYBUF, buf)
            self.buffers[i].length = buf.length
            self.buffers[i].start = mmap.mmap(
                self.vdw.fileno(),
                buf.length,
        ...
        for i in range(n_buffers):
            ...
            buf.memory = v4l2raw.V4L2_MEMORY_MMAP
            buf.index = i
            xioctl(self.vdr, v4l2raw.VIDIOC_QBUF, buf)

        if self.verbose:
            print("--requesetbuffers success")

        buftype = v4l2raw.v4l2_buf_type(v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE)
        xioctl(self.vd, v4l2raw.VIDIOC_STREAMON, buftype)

        time.sleep(1)
        buftype = v4l2raw.v4l2_buf_type(v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE)
        xioctl(self.vdr, v4l2raw.VIDIOC_STREAMON, buftype)
        if self.verbose:
            print("**** START ****")

    def Stop(self):
        buftype = v4l2raw.v4l2_buf_type(v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE)
        xioctl(self.vdr, v4l2raw.VIDIOC_STREAMOFF, buftype)
    ...
    def GetFrame(self):
        ...
        buf.type = v4l2raw.V4L2_BUF_TYPE_VIDEO_CAPTURE
        buf.memory = v4l2raw.V4L2_MEMORY_MMAP
        ret = xioctl(self.vdr, v4l2raw.VIDIOC_DQBUF, buf)
...
class oCams:
    def __init__(self, dev, verbose=0):
        ...
        self.Print(dev, 1)
        self.cam = v4l2_util(dev, verbose=verbose)
    ...
    def GetFrame(self, mode=0):
        ...
        if self.mode == 0:
            img = np.ndarray(
                (self.cam.shape[0], self.cam.shape[1], 2), buffer=buff, dtype=np.uint8
            )
            ...
            return np.ndarray(
                (self.cam.shape[0], self.cam.shape[1], 1), buffer=buff, dtype=np.uint8
            )
        elif self.mode == 3:
            return np.ndarray(
                (self.cam.shape[0], self.cam.shape[1] * 2, 1),
                buffer=buff,
                dtype=np.uint8,
            )
        else:
